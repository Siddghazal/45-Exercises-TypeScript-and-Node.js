...
let current_user: string[] = ["Shama","Hina","afia","fozia","shazia"];

let new_user: string[] = ["shama","hina","fiza","nida","sara"];

let currentUser=current_user.map(user=> user.toLowerCase())

for(let users of new_user){

  if (currentUser.includes(users.toLowerCase())){
    console.log(`sorry ${users} is not availale`);
  }
  else{
    console.log(`yes ${users} is available`);
  }
}
